...
function Home(props) {
	...
	const { data, loading, refetch } = useQuery(GET_MY_DOG, { fetchPolicy: "network-only" })
			...
			{dog && (
				<ScrollView style={styles.scroll}>
					...
					{dog?.diseases.length === 0 && (
						<VText style={styles.empty}>No hay enfermedades</VText>
					)}
					...
					{dog?.medicaments.length === 0 && (
						<VText style={styles.empty}>No hay medicamentos</VText>
					)}
					<View style={styles.list}>
					...
					{dog?.vaccines.length === 0 && (
						<VText style={styles.empty}>No hay vacunas</VText>
					)}
					<View style={styles.list}>
